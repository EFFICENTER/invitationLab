...
// Este código toma el atributo data-bs-img de la foto que clickeaste
// y lo pone en el modal
lightboxModal.addEventListener('show.bs.modal', function(event) {
    const button = event.relatedTarget; // La foto que clickeaste
    const imgSrc = button.getAttribute('data-bs-img'); // Su ruta
    lightboxImage.src = imgSrc; // Se la asigna al modal
});
...
const fechaObjetivoC = new Date(2026, 3, 25).getTime(); 

const countdownC = setInterval(() => {
    const ahora = new Date().getTime();
    const distancia = fechaObjetivoC - ahora;
    const d = Math.floor(distancia / (1000 * 60 * 60 * 24));
    const h = Math.floor((distancia % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60));
    const m = Math.floor((distancia % (1000 * 60 * 60)) / (1000 * 60));
    const s = Math.floor((distancia % (1000 * 60)) / 1000);
